...
Script containing to recreate De Bruyne, et al., 2021, Figure 5.
...
individual_prob_func = lambda x0, D, time, n: sum(
    [
        comb(N, m)
        * (erfc(x0 / np.sqrt(4 * D * time)) ** m)
        * (erf(x0 / np.sqrt(4 * D * time)) ** (N - m))
        for m in range(n)
    ]
)
